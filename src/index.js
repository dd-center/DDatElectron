...
open({ state, stateEmitter })
  .then(async db => {
    const { getWs, updateInterval, updateNickname, sendDanmaku } = await connect({ state, db })
    const quitAndInstall = async () => {
      await db.close()
      autoUpdater.quitAndInstall()
    }
    tray({ createWindow, getWin })
    sync({ getWin, updateInterval, updateNickname, state, stateEmitter, sendDanmaku, getWs, quitAndInstall, createWindow })
  })
